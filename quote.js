...
// recommended package with explanation, monthly outcome + cost, live sliders to
// adjust the mix, a branded printable quote, and a pre-filled email to Tal.
// Bilingual (all strings come from data attributes).
(function () {
  var calc = document.querySelector('.quote-calc');
  if (!calc) return;

  var tiers = JSON.parse(calc.getAttribute('data-tiers') || '[]');
  var marketing = JSON.parse(calc.getAttribute('data-marketing') || '[]');
  ...
  var croR = calc.querySelector('.qc-cro-range'), sdrR = calc.querySelector('.qc-sdr-range'), mkR = calc.querySelector('.qc-mk-range');
    ...
    chip.addEventListener('click', function () {
      ...
      if (set === 'tier') st.tier = +chip.getAttribute('data-tier');
        ...
        if (st.sdr) {
          ...
          if (!sel) { var f = calc.querySelector('.qc-chip[data-set="reps"]'); if (f) { f.classList.add('sel'); st.reps = +f.getAttribute('data-reps'); } }
  ...
  // sliders (fine-tune)
  if (croR) croR.addEventListener('input', function () { st.tier = +croR.value; renderResult(); });
  if (sdrR) sdrR.addEventListener('input', function () { var n = +sdrR.value; st.sdr = n > 0 ? 1 : 0; st.reps = n > 0 ? n : 1; renderResult(); });
  if (mkR) mkR.addEventListener('input', function () { var m = marketing[+mkR.value] || marketing[0]; st.mk = +m.mk; st.mkname = m.name; st.mkout = m.out; renderResult(); });
  ...
  function printQuote() {
    ...
    var css = 'body{font-family:Arial,Helvetica,sans-serif;color:#0b0f14;margin:0;padding:40px;background:#fff}' +
      '.q{max-width:640px;margin:0 auto}.brand{font-weight:800;font-size:30px;letter-spacing:1px}' +
      '.tag{color:#2563EB;font-weight:700;font-size:13px;text-transform:uppercase;letter-spacing:1px;margin-top:2px}' +
      '.bar{height:4px;background:#2563EB;border-radius:3px;margin:16px 0 24px}h1{font-size:22px;margin:0 0 4px}' +
      '.muted{color:#667;font-size:13px}.who{margin:14px 0;font-size:14px}' +
      '.rec{margin:22px 0 6px;font-size:12px;color:#2563EB;font-weight:700;text-transform:uppercase;letter-spacing:1px}' +
      '.recn{font-size:24px;font-weight:800;margin:0 0 8px}.why{color:#334;font-size:14px;margin:0 0 6px}' +
      ...
      '.terms{color:#667;font-size:12px;margin-top:22px}.foot{margin-top:20px;font-size:14px}.foot a{color:#2563EB}' +
      '@media print{.noprint{display:none}}.noprint{margin-top:26px}' +
      '.pbtn{background:#2563EB;color:#fff;border:0;border-radius:8px;padding:12px 20px;font-size:15px;font-weight:700;cursor:pointer}';
